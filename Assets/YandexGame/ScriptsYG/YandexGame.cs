...
namespace YG
{
    ...
    public partial class YandexGame : MonoBehaviour
    {
        ...
        [SerializeField] private bool pauseLB;
        [SerializeField] private float endlessTimer;
        private float timer;
        ...
        private bool isLeaderboardYG;
        public static bool isShowInter;
        #region Data Fields
        ...
        {
            switch (savesData.language)
            {
                case "ru":
                    LocalizationManager.SelectedLanguage = 1;
                    break;
                case "en":
                    LocalizationManager.SelectedLanguage = 0;
                    break;
                case "es":
                    LocalizationManager.SelectedLanguage = 2;
                    break;
                case "pt":
                    LocalizationManager.SelectedLanguage = 3;
                    break;
                case "tr":
                    LocalizationManager.SelectedLanguage = 4;
                    break;
                case "ja":
                    LocalizationManager.SelectedLanguage = 5;
                    break;
                default:
                    LocalizationManager.SelectedLanguage = 0;
                    break;
        ...
        public void Leaderboard(int value )
        {
#if !UNITY_EDITOR
           if (!isLeaderboardYG)
            {
                isLeaderboardYG = true;
                NewLeaderboardScores(leaderboardYG.nameLB, value);
                leaderboardYG.UpdateLB();
                Invoke("ReturneLeaderboard", 10);
            }
#endif
        ...
        void ReturneLeaderboard()
        {
            isLeaderboardYG = false;
        }
        ...
        [DllImport("__Internal")]
        public static extern void FullAdShow();

        public void _FullscreenShow()
        {
            if (timerShowAd >= infoYG.fullscreenAdInterval && !isShowInter)
            {
                isShowInter = true;
                timerShowAd = 0;
            ...
            {
                Message($"До запроcountа к показу рекламы в середине игры {(infoYG.fullscreenAdInterval - timerShowAd).ToString("00.0")} сек.");
            }
        }
